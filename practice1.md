...
# Установка необходимого ПО для работы с микроконтроллерами семейства nRF52
...
(в том числе и для Linux – скорее всего, версия gcc-arm-none-eabi в дистрибутиве будет не той, что нам нужна, а последняя версия – это 11.3.Rel1). Пользователи Mac OS X могут установить компилятор обычным способом, скачав пакет с расширением pkg соответственно.
...
```
cd ~
git clone https://git.miem.hse.ru/1405/RIOT.git
```
...а проверить, что сборка работает – вот так:
```
cd RIOT
git checkout 1405-master
cd apps/hello-world
make BOARD=nucleo-l152
```
...
Программа компилируется для отладочной платы nRF52-DK.
...
В "нативной" Ubuntu он ставится штатным образом:
```
sudo apt-get install openocd
```
В WSL вам понадобится скачать один из его дистрибутивов для Windows, например, отсюда (ссылка взята из раздела Getting OpenOCD на официальном сайте, так что можно считать это почти официальной версией):

https://freddiechopin.info/en/download/category/4-openocd

Распакуйте архив с OpenOCD 0.10.0, например, в корень диска C. Далее исполняемый файл скопируем, убрав расширение, и добавим путь в `.bashrc`:
```
cd /mnt/c/openocd-0.10.0/bin-x64/
cp openocd.exe openocd
echo "export PATH=/mnt/c/openocd-0.10.0/bin-x64/:\$PATH" >> ~/.bashrc
```

К сожалению, версия OpenOCD не самая новая, поэтому понадобится заменить файл `scripts/target/nrf52.cfg` в каталоге OpenOCD на приложенный:

[nrf52.cfg](uploads/5b103cf5e774eb3ff8da7143f5b9f1ee/nrf52.cfg)
...
В файле `hello-world/.vscode` присутствуют настройки IDE, позволяющие выполнять сборку и отладку проекта. Их можно использовать для дальнейшей работы, создавая такие же по аналогии для своих нужд.
...
При установке в Windows путь к компилятору добавится в переменную окружения PATH (если в конце установки вы не забудете выбрать нужную галочку), в Ubuntu и Mac OS X это надо сделать вручную.
